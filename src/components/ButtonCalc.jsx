...
const ButtonCalc = ({ selectedTarget, setSelectedTarget }) => {
  return (
    <div className="relative flex gap-2 bg-[#232526] p-2.5 rounded-3xl w-[30%]">
      
      {/* Set Target Button */}
      <button
        onClick={() => setSelectedTarget(0)}
        className={`flex-1 px-4 py-2 font-bold rounded-2xl transition-all duration-300 
        ${selectedTarget === 0 ? "bg-yellow-500 text-gray-100" : "bg-white text-black border border-blue-800"}`}
      >
        Set Target
      </button>
